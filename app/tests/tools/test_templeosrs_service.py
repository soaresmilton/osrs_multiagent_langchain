...
def test_get_player_stats_success(mocker):
    ...
    result = service.get_player_stats(username="Hoom Pk")

    assert result.success is True
    assert result.data["Overall_level"] == 2376

    mock_client.get_player_stats.assert_called_once_with(username="Hoom Pk")
...
def test_get_player_stats_exception(mocker):
    ...
    service = TempleOsrsService(client=mock_client)
    result = service.get_player_stats("Hoom Pk")
...
def test_get_player_info(mocker):
    mock_client = mocker.Mock()
    mock_client.get_player_info.return_value = {"Username": "Hoom Pk"}
